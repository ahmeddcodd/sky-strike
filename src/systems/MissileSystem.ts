...
import { paint, makeFlareTexture } from "../factories/MeshUtils";
...
function buildMissileSource(scene: Scene, hostile: boolean): Mesh {
  const parts: Mesh[] = [];
  ...
  const hull = hostile ? "#3c4654" : "#eef2f7";
  const accent = hostile ? "#ff5b3a" : "#35c8e8";

  const body = add(CreateCylinder("mBody", { height: 1.5, diameter: 0.24, tessellation: 10 }, scene), hull);
  body.rotation.x = Math.PI / 2;
  const nose = add(CreateCylinder("mNose", { height: 0.45, diameterTop: 0.02, diameterBottom: 0.24, tessellation: 10 }, scene), accent);
  nose.rotation.x = Math.PI / 2;
  nose.position.z = 0.95;
  const finV = add(CreateBox("mFinV", { width: 0.05, height: 0.62, depth: 0.32 }, scene), accent);
  finV.position.z = -0.58;
  const finH = add(CreateBox("mFinH", { width: 0.62, height: 0.05, depth: 0.32 }, scene), accent);
  finH.position.z = -0.58;

  const merged = Mesh.MergeMeshes(parts, true, true, undefined, false, false)!;
  merged.name = hostile ? "missileSrcHostile" : "missileSrcPlayer";
  const mat = new StandardMaterial(`missileMat${hostile ? "H" : "P"}`, scene);
...
export class MissileSystem {
  ...
  constructor(scene: Scene, vfx: VFXSystem, player: PlayerJet) {
    this.vfx = vfx;
    this.player = player;
    const glowHostile = makeGlowMat(scene, "missileGlowMatH", "#ffb066");
    const glowPlayer = makeGlowMat(scene, "missileGlowMatP", "#9fe8ff");
    const srcHostile = buildMissileSource(scene, true);
    const srcPlayer = buildMissileSource(scene, false);
